...
class _InflationScreenState extends State<InflationScreen> {
  ...
  double calculateLoanInflation(
      double loanAmount, double inflationRate, int loanDuration) {
    inflatedAmount = loanAmount;

    if (isYear) {
      for (int i = 0; i < loanDuration; i++) {
        inflatedAmount += inflatedAmount * (inflationRate / 100);
      }
    }
    if (isMonth) {
      for (int i = 0; i < (loanDuration / 12); i++) {
        inflatedAmount += inflatedAmount * (inflationRate / 100);
      }
    }
    setState(() {});
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                FocusScope.of(context).unfocus();
                                setState(() {});
                                double inflatedAmount = calculateLoanInflation(
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: Text(
                                      "If cost of an item in current year is ₹ ${_loanamountController.text} then after ${_loantermsController.text} ${isYear ? 'Yr' : 'Months'} that same item would cost ₹ ${formatter.format(inflatedAmount)}."),
                                ),
                              ],
                            ),
                            const Divider(),
