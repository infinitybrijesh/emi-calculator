...
class FixedDepositInterestPayoutScreen extends StatefulWidget {
  const FixedDepositInterestPayoutScreen({super.key});

  @override
  State<FixedDepositInterestPayoutScreen> createState() =>
      _FixedDepositInterestPayoutScreenState();
}

class _FixedDepositInterestPayoutScreenState
    extends State<FixedDepositInterestPayoutScreen> {
  ...
  bool isMonth = false;
  bool isMonthly = true;
  bool isQuaterly = false;
  double totalpayment = 0;
  var formatter = NumberFormat('#,##,###');
  double monthlyintrest = 0;
  double quaterlyintrest = 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          'Fixed Deposit Interest Payout (FD)',
          style: TextStyle(
                                ...
                                child: Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      isMonthly
                                          ? Text(
                                              "₹ ${formatter.format(monthlyintrest)}",
                                              style: const TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  color: Appcolor.primaryColor,
                                                  letterSpacing: 0.5),
                                          ...
                                          : Text(
                                              "₹ ${formatter.format(quaterlyintrest)}",
                                              style: const TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  color: Appcolor.primaryColor,
                                                  letterSpacing: 0.5),
  ...
  void _handleCalculation() {
    if (_depositamountController.text.isEmpty) {
    ...
    } else if (isMonthly == false && isQuaterly == false) {
      showSnackBar(
        context: context,
      ...
      double quterlyrate = rate / 4;
      quater = month / 3;
      monthlyintrest =
          double.parse(_depositamountController.text.replaceAll(',', '')) *
              double.parse(monthlyrate.toStringAsFixed(6));
      quaterlyintrest =
          double.parse(_depositamountController.text.replaceAll(',', '')) *
              double.parse(quterlyrate.toStringAsFixed(6));
      totalInterest =
          isMonthly ? monthlyintrest * month : quaterlyintrest * quater;
      totalDeposit =
          double.parse(_depositamountController.text.replaceAll(',', '')) +
              totalInterest;
      // log("bchjxbvchj " + monthlyintrest.toString());
